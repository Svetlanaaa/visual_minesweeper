...
class VisualSapper:
    ...
    def create_game_field(self):
        self.game_table = ScrolledFrame.ScrolledFrame(root, self.y * 36 + 240, [150, self.x * 28][self.x * 28 >= 150])
        for _ in range(self.x):
            self.buttons.append([Button(self.game_table.viewPort, width=1, heigh=1, bg='LightCyan') for i in range(self.y)])
        for i in range(self.x):
            for j in range(self.y):
                b = self.buttons[i][j]
        ...
        change_win_size(root, self.y * 36 + 240, [150, self.x * 28][self.x * 28 >= 150])
    ...
    def init_table(self, event):
        ...
        try:
            if 100 < self.x < 1 or 100 < self.y < 1 or self.x * self.y < self.z < 1:
